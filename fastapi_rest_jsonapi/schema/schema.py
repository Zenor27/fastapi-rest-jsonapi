...
class Schema(MarshmallowSchema):
    ...
    def __get_schema_class_from_field(self, field: str) -> type["Schema"]:
        ...
        field_schema_type: str = self.declared_fields[field].__dict__[
            "_Relationship__schema"
        ]
        field_schema_cls: list[type[Schema]] = next(
            filter(
                lambda schema_class: getattr(schema_class[0], "__type__", None)
                == field_schema_type,
    ...
    def __get_relationship_fields(self, fields: list[Field]) -> list[Field]:
        ...
        for field in fields:
            ...
            if declared_field := self.declared_fields.get(field_):
                if (
                    type(declared_field) is MarshmallowRelationship
                    or type(declared_field) is Relationship
                ):
    ...
    def dump(
        ...
        only = (
            [x.field for x in current_type_fields] + ["id"]
            if current_type_fields
            else None
        )
